...
ShortlistState shortlist_reducer(ShortlistState state, dynamic action) {
  ...
  if (action is AddShortlistResponse) {
    return add_shortlist_response(state, action);
  }
...
add_shortlist_response(ShortlistState state, dynamic action) {
  state.addShortlistResponse.result = action.result;
  state.addShortlistResponse.message = action.message;
  return state;
}
