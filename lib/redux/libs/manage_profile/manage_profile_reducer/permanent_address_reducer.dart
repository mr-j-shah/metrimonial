...
PermanentAddressState permanent_address_reducer(
  ...
  if (action is PermanentUpdatetResponse) {
    return permanent_update_response(state, action);
  }
...
state_response(PermanentAddressState state, PermanentAddressStateListAction action) {
  ...
  if(state.stateResponse.data.isNotEmpty){
    ...
    store.dispatch(cityMiddleware(state.selected_state.id, AppStates.permanentAddress));
  }
  return state;
}

city_responose(PermanentAddressState state, PermanentAddressCityListAction action) {
...
permanent_update_response(
    PermanentAddressState state, PermanentUpdatetResponse action) {
  state.permanentUpdatetResponse.result = action.result;
  state.permanentUpdatetResponse.message = action.message;
  return state;
}
