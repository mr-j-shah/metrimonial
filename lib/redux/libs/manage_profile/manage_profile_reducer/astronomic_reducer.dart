...
AstronomicState astronomic_reducer(AstronomicState state, dynamic action) {
  if (action is AstronomicInfoUpdateResponse) {
    return astronomic_info_update_response(state, action);
  }
  if (action is Loader) {
...
astronomic_info_update_response(
    AstronomicState state, AstronomicInfoUpdateResponse action) {
  state.astronomicInfoUpdateResponse.result = action.result;
  state.astronomicInfoUpdateResponse.message = action.message;
  return state;
}
