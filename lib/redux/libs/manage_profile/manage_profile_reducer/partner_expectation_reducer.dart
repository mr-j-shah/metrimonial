...
PartnerExpectationState partner_expectation_reducer(
  ...
  if (action is Pexsave) {
    return pex_save_toggler(state, action);
  }
...
pex_marital_status_value_add(PartnerExpectationState state, PexMaritalStatusAddValueAction action){
...
pex_partnerexpectation_get_response(
  ...
  state.general_requirement_controller.text = state.partnerExpectationGetResponse.data?.general;
  state.min_height_controller.text = "${state.partnerExpectationGetResponse.data.height??''}";
  state.max_weight_controller.text = "${state.partnerExpectationGetResponse.data.weight??''}";
  state.education_controller.text = "${state.partnerExpectationGetResponse.data.education??''}";
  state.profession_controller.text = "${state.partnerExpectationGetResponse.data.profession??''}";
  state.personal_value_controller.text = "${state.partnerExpectationGetResponse.data.personalValue??''}";
  state.complexion_controller.text = "${state.partnerExpectationGetResponse.data.complexion??''}";
  state.body_controller.text = "${state.partnerExpectationGetResponse.data.bodyType??''}";
...
pex_state_response(PartnerExpectationState state, StateResponseFromPartnerPref action) {
...
pex_sub_caste_response(PartnerExpectationState state, PartnerPrefSubCasteResponse action) {
...
pex_caste_response(PartnerExpectationState state, CasteResponseForPartnerPref action) {
  state.casteResponse.data.clear();

  state.casteResponse.data.addAll(action.data);


  if(state.casteResponse.data !=null && state.casteResponse.data.isNotEmpty) {
    ...
    if(state.partnerExpectationGetResponse.data?.casteId!=null) {
      state.casteResponse.data.forEach((element) {
        if (element.name == state.partnerExpectationGetResponse.data.casteId) {
          state.caste_val = element;
        }
      });
    }
    store.dispatch(subcasteMiddleware(state.caste_val.id,appStates: AppStates.partnerPreference));
  }
  return state;
}

pex_save_toggler(PartnerExpectationState state, Pexsave action) {
...
pexprofile_drop_down_response(PartnerExpectationState state) {
  ...
  store.state.manageProfileCombineState.profiledropdownResponseData.data.maritialStatus.forEach((element) {
     print(element.name == state.partnerExpectationGetResponse.data.maritalStatusId);
        // print(element.name == store.state.manageProfileCombineState.partnerExpectationState.partnerExpectationGetResponse.data.preferredCountryId);
    if(element.name == state.partnerExpectationGetResponse.data.maritalStatusId){
  ...
  if(state.partnerExpectationGetResponse.data?.religionId!=null) {
    store.state.manageProfileCombineState.profiledropdownResponseData.data
        .religionList.forEach((element) {
      if (element.name == state.partnerExpectationGetResponse.data.religionId) {
  ...
  if(state.partnerExpectationGetResponse.data?.languageId!=null) {
    store.state.manageProfileCombineState.profiledropdownResponseData.data
      ...
      print(element.name);
      if (element.name == state.partnerExpectationGetResponse.data.languageId) {
  ...
  if(state.partnerExpectationGetResponse.data?.diet!=null) {
    state.commonYesNoList.forEach((element) {
      if (element == state.partnerExpectationGetResponse.data.diet) {
  ...
  if(state.partnerExpectationGetResponse.data?.manglik!=null) {
    state.commonYesNoList.forEach((element) {
      if (element == state.partnerExpectationGetResponse.data.manglik) {
  ...
  if(state.partnerExpectationGetResponse.data?.familyValueId!=null) {
    store.state.manageProfileCombineState.profiledropdownResponseData.data
        .familyValueList.forEach((element) {
      if (element.name ==
