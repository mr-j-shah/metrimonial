...
SpiritualSocialState spiritual_reducer(
  ...
  if (action is SpiritualBackgroundUpdateResponse) {
    return spiritaul_background_update_response(state, action);
  }
...
spiritaul_background_update_response(
    SpiritualSocialState state, SpiritualBackgroundUpdateResponse action) {
  state.spiritualBackgroundUpdateResponse.result = action.result;
  state.spiritualBackgroundUpdateResponse.message = action.message;
  return state;
}
  ...
  if(state.subcasteResponse.data.isNotEmpty) {
    ...
    if (state.spiritualSocialGetResponse.result) {
      state.subcasteResponse.data.forEach((element) {
        if (element.name == state.spiritualSocialGetResponse.data.subCasteId) {
